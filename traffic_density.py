...
class VideoThumbnail(ttk.LabelFrame):
    ...
    def add_widgets(self):
        ...
        self.previewbutton = ttk.Button(self, text="Preview Video")
        self.previewbutton.pack(side="right", padx=10, pady=10)
    ...
    def runModel(self):
        ...
        if video is not None:
            traffic_density_player.main(tkinter.Toplevel(self),"videos\\"+video)
            self.connection.commit()
            self.callback()
...
class ModelResults(ttk.LabelFrame):
    ...
    def update(self):
        ...
        self.tree = ttk.Treeview(
            self,
            columns=("Density","Classification","Location","Intersection","Datetime"),
            height=11,
            selectmode="browse",
            show=("headings",),
            yscrollcommand=self.scrollbar.set,
        ...
        self.tree.heading("Intersection", text="Intersection")
        self.tree.heading("DateTime", text="DateTime")

        self.tree.column("Density", anchor="w")
        self.tree.column("Classification", anchor="w")
        self.tree.column("Location", anchor="w")
        self.tree.column("Intersection", anchor="w")
        self.tree.column("Datetime", anchor="w")
        query = "SELECT density, classification, location, intersection, creation_time FROM temp2"
        cursor = self.connection.cursor()
        cursor.execute(query)
        rows = cursor.fetchall()
        for idx, row in enumerate(rows):
...
def main(window):
    root = window
    root.title("Traffic Density Detection Model")
